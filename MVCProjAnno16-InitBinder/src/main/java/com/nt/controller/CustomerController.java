...
public class CustomerController {
	...
	public String registerCustomer(@ModelAttribute("cust") Customer cust,
			BindingResult errors, RedirectAttributes redirectAttrs) { // Post
																		// back
																		// request
																		// (form
																		// submission)

		// pefrom server side form validations only when client side form
		// valdations are not done
	...
	public List<String> populateHobies() {
		...
		return List.of("Reading Books", "Watching TV", "Sleeeping",
				"Playing Games");
	}
	...
	public void myInitBinder(ServletRequestDataBinder binder) {
		...
		// makes the binder obj to use CustomDateEditor to convert given String
		// value(yyyy-MM-dd) to java.util.Date class obj
