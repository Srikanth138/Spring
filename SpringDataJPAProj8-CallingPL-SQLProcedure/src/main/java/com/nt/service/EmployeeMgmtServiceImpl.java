...
public class EmployeeMgmtServiceImpl implements IEmployeeMgmtService {
	...
	/*
	  CREATE OR REPLACE PROCEDURE P_GET_EMPS_BY_SALRANGE (STARTSALARY IN NUMBER ,ENDSALARY IN NUMBER , DETAILS OUT SYS_REFCURSOR ) 
	  AS 
	  BEGIN
	  OPEN DETAILS FOR SELECT ENO,ENAME,EADD,ESALARY FROM EMPLOYEE WHERE
	  ESALARY>=STARTSALARY AND ESALARY<=ENDSALARY;
	  END ;
	  /
	 */
	...
	/*
	 CREATE OR REPLACE PROCEDURE P_GET_EMPS_BY_CITIES (CITY1 IN VARCHAR2 , CITY2 IN VARCHAR2 , DETAILS OUT SYS_REFCURSOR ) 
	 AS 
	 BEGIN 
	 OPEN DETAILS FOR SELECTE NO,ENAME,EADD FROM EMPLOYEE WHERE EADD IN(CITY1,CITY2);
	 END 
	 P_GET_EMPS_BY_CITIES;
	 /
	 */
	...
	/*
	 CREATE OR REPLACE FUNCTION FX_AUTHENTICATION( UNAME IN VARCHAR2 , PWD IN VARCHAR2) RETURN VARCHAR2 AS CNT NUMBER(5); RESULT VARCHAR2(20); 
	 BEGIN 
	 SELECT COUNT(*) INTO CNT FROM USERSLIST WHERE USERNAME=UNAME AND PASSWORD=PWD; 
	 IF(CNT<>0) THEN RESULT:='VALID CREDENTIALS'; 
	 ELSE
