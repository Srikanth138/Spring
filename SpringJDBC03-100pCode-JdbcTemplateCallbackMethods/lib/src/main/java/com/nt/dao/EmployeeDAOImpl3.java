...
public class EmployeeDAOImpl3 implements IEmployeeDAO {
	...
	public List<EmployeeBO> getEmployeesByDesgs(String desg1, String desg2,
			String desg3) {
		BeanPropertyRowMapper<EmployeeBO> rowMapper = new BeanPropertyRowMapper(
				EmployeeBO.class);
		List<EmployeeBO> listBO = jt.query(GET_EMPS_BY_DESG,
				new RowMapperResultSetExtractor<EmployeeBO>(rowMapper), desg1,
				desg2, desg3);
		return listBO;

		/*
		 * return jt.query(GET_EMPS_BY_DESG, new
		 * RowMapperResultSetExtractor<EmployeeBO>(new
		 * BeanPropertyRowMapper<EmployeeBO>(EmployeeBO.class)),
		 * desg1,desg2,desg3);
		 */
	}
}
