...
public class EmployeeMgmtServiceImpl implements IEmployeeMgmtService {
	...
	@Override
	public List<Map<String, Object>> fetchEmployeeDetailsByDesgs(String desg1,
			String desg2, String desg3) {
		// use DAO
		List<Map<String, Object>> list = dao.getEmployeeDetailsByDesgs(desg1,
				desg2, desg3);
		return list;
	}

	@Override
	public SqlRowSet fetchEmployeeDetailsBySalRange(float startSalary,
			float endSalary) {
		// use DAO
		SqlRowSet rowset = dao.getEmployeeDetailsBySalaryRange(startSalary,
				endSalary);
		return rowset;
	}
	...
	public String hikeEmployeeSalary(int eno, float percentage) {
		...
		return count == 1
				? "Employee Salary updated with ::" + newSalary
				: "Employee salary not updated";
	}

	@Override
	public String fireEmployeesBySalaryRange(float startSalary,
			float endSalary) {
		int count = dao.deleteEmployeeBySalaryRange(startSalary, endSalary);
		return count == 0
				? "No employees found for deletion"
				: count + "no.of employees are found and deleted";
	}
