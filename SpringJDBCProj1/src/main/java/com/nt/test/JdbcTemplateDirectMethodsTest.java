...
public class JdbcTemplateDirectMethodsTest {
	public static void main(String args[]) {
		...
		try {
			System.out.println(
					"Emp salary::" + service.fetchEmpSalaryByEno(7499));
		} catch (DataAccessException dae) {
			dae.printStackTrace();
		}
		...
		System.out.println("=============================");
		try {
			// System.out.println("7499 Emp Details are::"
			// + service.fetchEmployeeDetailsByEno(7499));
		...
		try {
			...
			// are::"+service.fetchEmployeeDetailsByDesgs("CLERK","MANAGER","SALESMAN"));
			System.out.println("CLERK Desg Emp Details are::");
			service.fetchEmployeeDetailsByDesgs("CLERK", "MANAGER", "SALESMAN")
					.forEach(map -> {
						map.entrySet().forEach(row -> {
							System.out.println(
		...
		try {
			...
			SqlRowSet rowset = service.fetchEmployeeDetailsBySalRange(2000.0f,
					6000.0f);
			while (rowset.next()) {
